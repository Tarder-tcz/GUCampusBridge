...
export const PostCard = ({ post }) => {
      ...
      >
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <button
            ...
            className={`flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-semibold font-mono transition-colors ${
              isUpvoted
                ? 'bg-slate-800 text-slate-100 border border-slate-700'
                : 'bg-slate-900/80 text-slate-400 hover:text-slate-200 border border-slate-800'
            }`}
          >
            <ArrowBigUp className={`w-4 h-4 ${isUpvoted ? 'fill-slate-100 text-slate-100' : ''}`} />
        ...
        <div className="flex items-center gap-3 text-xs text-slate-400 shrink-0 font-mono">
          ...
          <span className="w-12 text-right">💬 {post.commentCount}</span>
          <a
            href={`/post/${post.id}`}
            target="_blank"
  ...
  // Reddit Card View Mode
  return (
    <article className="glass-card rounded-2xl p-4 sm:p-5 flex gap-4 border border-slate-800/80 hover:border-slate-700 transition-all relative">
      ...
      <div className="flex flex-col items-center shrink-0">
        <button
          ...
          className={`p-1.5 rounded-xl transition-all ${
            isUpvoted
              ? 'bg-slate-800 text-slate-100 border border-slate-700 scale-105'
              : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'
          }`}
          title="Upvote"
        >
          <ArrowBigUp className={`w-6 h-6 ${isUpvoted ? 'fill-slate-100' : ''}`} />
        </button>

        <span className="text-xs font-bold font-mono my-1 text-slate-200">
          {post.votes}
        </span>
        ...
        <button
          ...
          title="Downvote"
        >
          <ArrowBigDown className={`w-6 h-6 ${isDownvoted ? 'fill-slate-100' : ''}`} />
        </button>
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between gap-2 mb-2 flex-wrap text-xs">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-[11px] font-mono font-semibold text-slate-400 bg-slate-900/90 px-2 py-0.5 rounded-md border border-slate-800">
              {post.channelName}
            </span>
